...
const PLATFORMS = [
  {id:'instagram',name:'Instagram',icon:'ð¸œ¸',bg:'linear-gradient(135deg,#f09433,#dc2743,#bc1888)',maxChars:2200,note:'Add to Stories + Reels'},
  {id:'tiktok',name:'TikTok',icon:'ð¸½µ',bg:'linear-gradient(135deg,#010101,#69C9D0)',maxChars:2200,note:'Hook in first line'},
  {id:'youtube',name:'YouTube',icon:'â“¶ï¸',bg:'linear-gradient(135deg,#FF0000,#cc0000)',maxChars:500,note:'Shorts caption'},
  {id:'facebook',name:'Facebook',icon:'ð¸˜¥',bg:'linear-gradient(135deg,#1877F2,#0d5bba)',maxChars:63206,note:'End with a question'},
];

const WEEKLY_SCHEDULE = [
  {
    day:'Monday', continent:'Europe', color:'#06b6d4', emoji:'ð¸¡ªð¸¡º',
    topics:[
  ...
  {
    day:'Tuesday', continent:'Asia', color:'#a855f7', emoji:'ð¸’',
    topics:[
  ...
  {
    day:'Wednesday', continent:'USA', color:'#FF6B2B', emoji:'ð¸¡ºð¸¡¸',
    topics:[
  ...
  {
    day:'Thursday', continent:'Tips & Guides', color:'#f59e0b', emoji:'ð¸™¡',
    topics:[
  ...
  {
    day:'Friday', continent:'Weekend Inspiration', color:'#f43f5e', emoji:'â“†ï¸',
    topics:[
      ...
      {title:'How to plan the perfect theme park trip for under âš¬200', category:'Tips', tone:'informative'},
    ]
  },
  {
    day:'Saturday', continent:'Americas + Middle East + Africa', color:'#10b981', emoji:'ð¸’',
    topics:[
  ...
  {
    day:'Sunday', continent:'Oceania + Global News', color:'#06b6d4', emoji:'ð¸’ ',
    topics:[
...
export default function AgentPage() {
  ...
  const publishToSite = async () => {
    ...
    const emojis = {'Park Guide':'ð¸”ºï¸','Comparison':'â¡ï¸','News':'ð¸œ°','Tips':'ð¸™¡','Hidden Gems':'ð¸™½','Top Lists':'ð¸ ','Destination':'â“†ï¸','App Updates':'ð¸œ±'};
    const post = {slug,title:blog.title||'',excerpt:blog.excerpt||'',category,emoji:emojis[category]||'ð¸½¢',date:new Date().toISOString().split('T')[0],readTime:blog.readTime||'5 min',content:blog.content||''};
    try {
...
Return ONLY a valid JSON object (no markdown, no backticks):
{
  ...
  "hashtags": ["funparks","themeparks","rollercoaster","themepark","coasters","disney","universalstudios","amusementpark","themepark2025","parklife","coasterlife","familytravel","traveleurope","thrillseeker","weekendtrip"]
    ...
    try {
      ...
      let parsed;try{parsed=JSON.parse(clean);}catch(e1){try{clean=clean.replace(/[\\u0000-\\u001F\\u007F-\\u009F]/g,' ').replace(/\\n/g,' ').replace(/\\r/g,' ');parsed=JSON.parse(clean);}catch(e2){throw new Error('JSON parse failed: '+e2.message.substring(0,100));}}
    ...
    <div style={{minHeight:'100vh',background:'#f8f7ff',fontFamily:'Plus Jakarta Sans,sans-serif'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#1a1a2e,#2d1b69)',padding:'20px 32px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{fontFamily:'Syne,sans-serif',fontSize:'20px',fontWeight:'800',color:'white'}}>
          ...
          <span style={{color:'rgba(255,255,255,0.6)',fontSize:'14px',fontWeight:'600',marginLeft:'12px'}}>Content Pipeline ð¸½¢</span>
        </div>
      ...
      <div style={{maxWidth:'1100px',margin:'0 auto',padding:'28px 20px'}}>
        ...
        <div style={{display:'flex',gap:'4px',background:'white',borderRadius:'16px',padding:'5px',border:'2px solid #f0eeff',marginBottom:'24px',width:'fit-content',boxShadow:'0 2px 12px rgba(168,85,247,0.08)'}}>
          {[['schedule','ð¸œ¦ Weekly Schedule'],['generate','â“¨ Generate'],['review','ð¸œ Review & Edit'],['publish','ð¸¡¬ Publish']].map(([id,label])=>(
        ...
        {tab==='schedule' && (
          <div>
            <div style={{marginBottom:'20px'}}>
              <h2 style={{fontFamily:'Syne,sans-serif',fontSize:'18px',fontWeight:'900',color:'#1a1a2e',marginBottom:'6px'}}>ð¸œ¦ Weekly Content Calendar</h2>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:'flex',gap:'8px'}}>
                        ...
                        <span style={{fontSize:'11px',fontWeight:'600',color:'#9ca3af',padding:'2px 8px',borderRadius:'999px',background:'#f3f4f6'}}>
                          {{fun:'ð¸½° Fun',informative:'ð¸œ¡ Info',inspiring:'â“¨ Inspiring',debate:'ð¸¥ Debate'}[t.tone]}
                        </span>
                      </div>
                    </div>
            ...
            <div style={{marginTop:'20px',background:'white',borderRadius:'16px',padding:'20px',border:'2px solid #f0eeff'}}>
              <p style={{fontFamily:'Syne,sans-serif',fontSize:'13px',fontWeight:'800',color:'#1a1a2e',marginBottom:'12px'}}>ð¸™° Affiliate Links (auto-included in all content)</p>
        ...
        {tab==='generate' && (
          <div>
            <div style={{background:'white',borderRadius:'20px',padding:'24px',marginBottom:'22px',border:'2px solid #f0eeff',boxShadow:'0 4px 20px rgba(168,85,247,0.06)'}}>
              <h2 style={{fontFamily:'Syne,sans-serif',fontSize:'17px',fontWeight:'800',marginBottom:'16px'}}>â“¨ Generate Blog Post + Social Media Content</h2>
              <div style={{display:'flex',gap:'12px',flexWrap:'wrap',alignItems:'flex-end'}}>
                ...
                <div>
                  ...
                  <select value={tone} onChange={e=>setTone(e.target.value)}
                    style={{border:'2px solid #e5e7eb',borderRadius:'10px',padding:'9px 13px',fontSize:'13px',fontFamily:'inherit',color:'#1a1a2e',outline:'none'}}>
                    <option value="fun">ð¸½° Fun & Energetic</option>
                    <option value="informative">ð¸œ¡ Informative</option>
                    <option value="inspiring">â“¨ Inspiring</option>
                    <option value="debate">ð¸¥ Debate Starter</option>
                  </select>
                </div>
                <button onClick={generate} disabled={loading}
                  style={{...btnStyle('orange'),padding:'11px 24px',fontSize:'13px',opacity:loading?0.6:1,cursor:loading?'not-allowed':'pointer'}}>
                  {loading ? 'â³ Generating...' : 'â“¨ Generate'}
                </button>
              </div>
            </div>
            ...
            {blog && !loading && (
              <div style={{background:'#d1fae5',border:'2px solid #34d399',borderRadius:'16px',padding:'16px 20px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div>
                  <p style={{fontWeight:'800',color:'#065f46'}}>â“¦ Content generated!</p>
          ...
          <div>
            <div style={{background:'white',borderRadius:'20px',border:'2px solid #f0eeff',overflow:'hidden',marginBottom:'22px'}}>
              <div style={{padding:'14px 22px',background:'linear-gradient(135deg,#fff7f5,#fdf4ff)',borderBottom:'2px solid #f0eeff',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <span style={{fontFamily:'Syne,sans-serif',fontSize:'14px',fontWeight:'800'}}>ð¸œ Blog Post â¬ funparks.app/blog/{slug}</span>
                <div style={{display:'flex',gap:'8px'}}>
            ...
            <p style={{fontWeight:'800',fontSize:'14px',marginBottom:'12px',color:'#1a1a2e'}}>ð¸œ± Social Media Posts</p>
                  ...
                  <div key={p.id} style={{background:'white',borderRadius:'20px',padding:'18px',border:'2px solid #f0eeff'}}>
                    ...
                    <div style={{display:'flex',gap:'7px',marginTop:'10px'}}>
                      <button style={{...btnStyle(),fontSize:'12px'}} onClick={()=>copyText(posts[p.id]||'','post_'+p.id)}>
                        {copied['post_'+p.id] ? 'â“¦ Copied!' : 'ð¸œ¹ Copy'}
                      </button>
                    </div>
                  </div>
                );
        ...
        {tab==='review' && !blog && (
          <div style={{textAlign:'center',padding:'60px 24px',color:'#9ca3af'}}>
            <div style={{fontSize:'48px',marginBottom:'12px'}}>ð¸œ</div>
        ...
        {tab==='publish' && (
          <div>
            <div style={{background:'white',borderRadius:'20px',padding:'22px',border:'2px solid #f0eeff',marginBottom:'20px'}}>
              <h2 style={{fontFamily:'Syne,sans-serif',fontSize:'17px',fontWeight:'800',marginBottom:'8px'}}>ð¸¡¬ Publish to funparks.app</h2>
                  ...
                  <div style={{background:'#1a1a2e',borderRadius:'14px',padding:'16px',marginBottom:'14px'}}>
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
                      ...
                      <button style={{...btnStyle('green'),fontSize:'12px'}} onClick={()=>copyText(script,'script')}>
                        {copied.script ? 'â“¦ Copied!' : 'ð¸œ¹ Copy Script'}
                      </button>
                    </div>
