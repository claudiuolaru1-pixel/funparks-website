...
const write=(p,c)=>{fs.mkdirSync(path.dirname(p),{recursive:true});fs.writeFileSync(p,c,'utf8');console.log('wrote:',p.replace(site,''));};

// API ROUTE - server side, key is safe here
write(site+'/app/api/generate/route.js', `
...
console.log('\nAll done! Now run: git add . && git commit -m "Add content agent" && git push');